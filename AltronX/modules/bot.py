...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"»⚡️𝗚𝗔𝗡𝗚𝗦𝗧𝗘𝗥⚡️_ᴏᴘ_ʙᴏʟᴛᴀ", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"_⚡️𝗚𝗔𝗡𝗚𝗦𝗧𝗘𝗥 𝗙𝗨𝗖𝗞𝗜𝗡𝗚 𝗠𝗢𝗗 𝗢𝗡⚡️_\n» `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __ᴇᴋ ɴᴀʏᴀ⚡️𝗚𝗔𝗡𝗚𝗦𝗧𝗘𝗥'𝗦⚡️ʙᴇᴛᴀ ᴀᴅᴅ ʜᴏ ʀʜᴀ...__")
        mks = "SUDO_USER"
        target = ""
        ...
        if len(sudousers) > 0:
            ...
            newsudo = f"{target}"
        await ok.edit(f"» **ɴᴇᴡ⚡️𝗚𝗔𝗡𝗚𝗦𝗧𝗘𝗥'𝗦 ⚡️ʙᴇᴛᴀ**: `{target}`\n» `ʙᴏᴛ ғɪʀ sᴇ sᴜʀᴜ ʜᴏ ʀʜᴀ...`")
